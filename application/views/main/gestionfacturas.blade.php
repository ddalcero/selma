...
<?php echo Table::body($pendientes->results)
        ->ignore('estado','user_id','fecha_fac','lot_id','tipo_dte','nr_dte','clt_id','spj_id','detalle','tasa_iva','iva','total','valor_uf')
        ...
        ->buttondel(function($pendiente){return "<a href='/solicitud/".$pendiente->id."' data-method='delete' class='btn btn-warning'><i class='fa fa-trash'></i> </a>";})
        ->order('id','meta','fecha_sol','cliente','glosa','importe_clp','total_uf','button','buttondel');
?>
{{ Table::close() }}
{{ $pendientes->links(); }}
@else
